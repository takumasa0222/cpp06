...
- static_cast: ダウンキャスト用(親から子)で使用できる際はこれを使う。実行時コストなし。変換元と先のクラスが親と子の関係でなければコンパイルエラーが発生する。コンパイル時の方しか確認しないため、実行時の本当の型は判別できない。
...
## 参照型に対しての dynamic_cast ##
参照にはポインターのような何も参照していない状態が存在しない。
このため、dynamic_cast を実行した際に、もとの実体とことなる場合 bad_cast 例外が発生する。

## RTTI ##
Run-Time Type Information のこと。C++ が提供する機能で、実行時にオブジェクトの型情報を取得することができる。